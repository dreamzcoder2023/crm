...
@section('content')
    ...
    <div style="margin-top: 30px;">
        <h4 class="fw-bold py-3 mb-4" style="margin-top:-49px;font-size:16px;color:black;">
            <span class="fw-light">Payment
        </h4>
        <div class="row" style="position:absolute; top:90px; right:50px ">
            <div class="col-md-12">
                @can('payment-create')
                    <ul class="nav nav-pills flex-column flex-md-row mb-3">
                        <li class="nav-item"><a class="nav-link active" href="{{ route('payment-create') }}"><i
                                    class="bx bxs-payment me-1"></i> Add payment</a></li>
        ...
        <form id="submit-form">
            <div class="d-flex justify-content-between align-items-center m-2">
                ...
                <div class="d-flex justify-content-center align-items-center">
                    ...
                    <a class="btn btn-danger" href="{{ route('payment-index') }}">
                    ...
                    @foreach ($payments as $payment)
                        <tr>
                            ...
                            @canany(['payment-edit', 'payment-delete'])
                                <td>
                                    @can(['payment-edit'])
                                        <a class="" href="{{ route('payment-edit', $payment->id) }}"><i
                                    ...
                                    @can('payment-delete')
                                        @if (!in_array($payment->id, $wallet) && !in_array($payment->id, $transfer) && !in_array($payment->id, $expenses))
                                            <a data-toggle="modal" href="javascript:void(0)" data-id="{{ $payment->id }}"
                                                class="deletepayment"><i class="bi bi-trash" style="font-size:24px; color:red"></i>
                                            </a><br />
                                        @endif
                                    @endcan
                                </td>
