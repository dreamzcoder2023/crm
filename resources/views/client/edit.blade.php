...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light">Edit Client
</h4>
...
<div class="row">
<form name="EditClient" action="{{route('client.update',$client->id)}}" id="editClient" method="post" >
            @csrf
            {{ method_field('PUT') }}
  <div class="col-xl">
      ...
      <div class="card-body">
        <div class="row">
            ...
            <input type="text" id="first_name" name="first_name" class="form-control" id="basic-default-fullname" onkeydown="return /[a-z, ]/i.test(event.key)"
                ...
                onfocus="if (this.value == '') {this.value = '';}" placeholder="Enter First name"  value="{{$client->first_name}}"/>
          ...
          <div class="mb-3">
            ...
            onblur="if (this.value == '') {this.value = '';}"
            onfocus="if (this.value == '') {this.value = '';}" placeholder="Enter Last name" value="{{$client->last_name}}" />
            <label id="last-error" class="error" for="basic-default-last_name">last Name is required</label>
          </div>
          <div class="mb-3">
            <label class="form-label" for="basic-default-email">Email</label>

              <input  type="text" id="email" name="email" class="form-control" placeholder="Enter Email"  value="{{$client->email}}"/>
            ...
            <input type="text"  name="company_name"  class="form-control phone-mask"  placeholder="Enter company name" value="{{$client->company_name}}" />
    <!-- <label id="phone-error" class="error" for="basic-default-phone">Phone number is required</label> -->
            ...
            <input type="text" maxlength="10" name="phone" value="{{$client->phone}}" id="phone" class="form-control phone-mask" onkeydown="return /[0-9, ]/i.test(event.key)"
    onblur="if (this.value == '') {this.value = '';}"
    onfocus="if (this.value == '') {this.value = '';}" placeholder="Enter Phone number" />
    <label id="phone-error" class="error" for="basic-default-phone">Phone number is required</label>
          </div>

          <div class="mb-3">
            <label class="form-label" for="basic-default-phone">Address</label>
            <textarea type="text" name="address"  class="form-control phone-mask" placeholder="Enter address" >{{$client->address}}</textarea>
