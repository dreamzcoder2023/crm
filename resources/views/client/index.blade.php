            ...
            <div class="col-md-12">
                @can('client-create')
                    <ul class="nav nav-pills flex-column flex-md-row mb-3">
                        <li class="nav-item"><a class="nav-link active" href="{{ route('client-create') }}"><i
                                    class="bx bx-user me-1"></i> Add Client</a></li>
            ...
            <table class="table" id="client_listing_table">
                ...
                <tbody class="table-border-bottom-0">
                  @if(count($clients) > 0)
                    @foreach ($clients as $client)
                        <tr>
                            ...
                            @canany(['client-edit', 'client-delete', 'client-view'])
                                <td>
                                    @can(['client-view'])
                                        <a href="{{ route('client-show', $client->id) }}" data-id="{{ $client->id }}"
                                            class=""><i class="bi bi-eye" style="font-size:24px"></i></a>
                                    @endcan
                                    @can('client-edit')
                                    ...
                                    @can('client-delete')
                                        @if (!in_array($client->id, $project) && !in_array($client->id, $wallet))
                                            <a data-toggle="modal" href="javascript:void(0)" data-id="{{ $client->id }}"
                                                class="deleteClient"><i class="bi bi-trash" style="font-size:24px; color:red"></i>
                                            </a><br />
                                        @endif
                                    @endcan
                                </td>
                    ...
                    @endforeach
                    @else
                    <tr><td colspan="7"><center>No found data.</center></td></tr>
                    @endif
                </tbody>
            </table>
